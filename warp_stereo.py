...
def mcbb(s1, s2, H1, H2):
    ...
    minx = np.floor(min(corners_x[0, :])).astype(int)
    maxx = np.ceil(max(corners_x[0, :])).astype(int)
    miny = np.floor(min(corners_x[1, :])).astype(int)
    maxy = np.ceil(max(corners_x[1, :])).astype(int)
